...
void ECVTMaintenance::updateWindowData()//slot
{
	...
	case EC4T_SUBSTRATE_ALIGN:
		...
		EC4T_substrate_align__update_oa_baseline_result_p_matching_rate_qle->setText(QString("%1").arg(SMEE_SI_TO_PCT(u_param.param_EC4T_substrate_align.update_oa_baseline_result_p.matching_rate),0,'f',3));
	...
	case EC4T_UPDATE_WA_BASELINE:
		...
		EC4T_update_wa_baseline__oa_baseline_result_p_matching_rate_qle->setText(QString("%1").arg(SMEE_SI_TO_PCT(u_param.param_EC4T_update_wa_baseline.oa_baseline_result_p.matching_rate),0,'f',3));
		break;
        //输出信息到输出栏
...
void ECVTMaintenance::performButtonClickedSlot(int button_id)
{
	...
	case (int)EC4T_SUBSTRATE_ALIGN:
		if(EC4T_substrate_align__oa_align_recipe_template_file_qle->text() == NULL)
		{
			SMEE::MessageBox::critical(this,"Error","The file name is null.");
		}
		else if( m_spinBoxAgent->CheckIsValid(EC4T_substrate_align__nr_marks_qsb) &&
	...
	case (int)EC4T_RETICLE_ALIGN:
		if(EC4T_reticle_align__align_spec_template_file_qle->text() == NULL)
		{
			SMEE::MessageBox::critical(this,"Error","The file name is null.");
		}
		else
	...
	case (int)EC4T_WA_ALIGN:
		if(EC4T_wa_align__oa_align_recipe_template_file_qle->text() == NULL)
		{
			SMEE::MessageBox::critical(this,"Error","The file name is null.");
		}
		else
	...
	case (int)EC4T_TTL_ALIGN:
		if(EC4T_TTL_align__ttl_align_recipe_template_file_qle->text() == NULL)
		{
			SMEE::MessageBox::critical(this,"Error","The file name is null.");
		}
		else
