...
def global_varibale(workbook,CCNAME,log):
...
#--------------------------------------------------------------------
    # 用于判断头文件包含文件。
    for i in range(len(funs_list_sh1)):
        if CCNAME + "4A" == funs_list_sh1[i][0:4]:
            global bool_XX4A
            bool_XX4A = True
            continue
        else:
            pass
        if CCNAME + "4T" == funs_list_sh1[i][0:4]:
            global bool_XX4T
            bool_XX4T = True
            continue
        else:
...
def check_funs(log):
    if len(set_fun_sh1) != len(funs_list_sh1):
        log.write("error:sheet1 页存在重复的接口！\n")
    else:
        #接口列表写到临时文件，显示在qt界面。
        file_funslist=open('temp_funslist.txt','w')
        for fun in funs_list_sh1:
            file_funslist.write(fun+'\n')
    if len(set_fun_sh2) != len(funs_list_sh2):
...
def read_excel_index2(workbook,log):
    ...
    excelshee2_info = []
    tempLineIndex = 0
    print("1.2、the information of excel index2:")
    sh2 = workbook.sheet_by_index(2)
    colnum2 = sh2.ncols
            ...
            if sh2.cell(i,j).value =='':
                ...
                if j == 4:
                    if '_ENUM' in sh2.cell(i,4).value:
                        ...
                        dict_enumUi.append(str((sh2.cell(i,j).value)).strip(' '))
...
#------------------------------------------------------------------
    ...
    print("temp_single_set=", list(temp_single_set))
