...
def excel_to_xml(curpath):
    fp = open((os.sep).join([curpath,'XXVT', 'excelFunsXml.xml']), 'w')
    doc = Document()  # 在内存中创建dom文档
    fun_info_root = doc.createElement('fun_info')  # 创建根节点fun_info
    doc.appendChild(fun_info_root)  # 根节点添加到dom树。

    for index in range(len(checkExcel.funs_list_sh1_tuple)):#第一层循环，接口个数。
        nodeFunName = doc.createElement('fun_name')
        funName = list(checkExcel.dict_funsShee2.keys())[index]
        nodeFunName.setAttribute('name', funName)
        funs_value = checkExcel.dict_funsShee2[funName]
...
def excelToXmlForUi(workbook,pageList1Index,
    ...
    fp = open((os.sep).join([curpath,'XXVT','excelXmlForUi.xml']), 'w')
    doc = Document()  # 在内存中创建dom文档
...
def xmlToBottomCpp(CCNAME,
    ...
    DOMTree = xml.dom.minidom.parse(curpath+"/"+"XXVT/excelFunsXml.xml")
    funInfo = DOMTree.documentElement
    funNames = funInfo.getElementsByTagName("fun_name")
    #print('funNames=',funNames[0])
    fileIni = open((os.sep).join([filepath, "outcodefiles", "VT" + CCNAME + '.ini']), 'w')
    ...
    for pageIndex in range(len(list(dict_pages.keys()))):
        ...
        bottomFile = open((os.sep).join([filepath, "outcodefiles", "bottom"+CCNAME + "VT" + pages + ".cpp"]), 'a+')#可读可写
        fileRange = open((os.sep).join([filepath, "outcodefiles", 'temp_rangeCheck.txt']), 'a+')
        fileButtonClick = open((os.sep).join([filepath, "outcodefiles", 'performButtonClickedSlot.txt']), 'a+')
        fileExec = open((os.sep).join([filepath, "outcodefiles", 'm_exec.txt']), 'a+')
...
'''%(CCNAME,pages))
	...
	\tswitch (button_id)\n\t\t{\n'''%(CCNAME,pages))
        ...
        fileButtonClick.write('\n\t\t\t\tbreak;\n\t\t}\n\t}')
        fileIni.write('#end\n')
        cppfilepath = generateCodefile.pageTopCppCode(CCNAME,filepath,dict_pages,pages)
