...
def preprocess_header_files(CCNAME,filepath):
    print("collectExcelInfo.bool_XX4A=", collectExcelInfo.bool_XX4A)
    print("collectExcelInfo.bool_XX4T=", collectExcelInfo.bool_XX4T)
    if collectExcelInfo.bool_XX4A == True:
    ...
    if collectExcelInfo.bool_XX4T == True:
...
def generate_requirefuns_tempfile(CCNAME, filepath,log):
    ...
    for i in range(len(collectExcelInfo.funslist)):
        temp_parameter_list = []
        if CCNAME + '4A' == collectExcelInfo.funslist[i][0:4]:  # CC4A接口
            file1 = open((os.sep).join([filepath,"temp_HeaderFiles", "pre" + CCNAME + "4A_if.h"]), 'r')
            file1.seek(0)
            ...
            while True:
                ...
                if line_info1.startswith('SMEE_EXPORT'):  # 找到以SMEE_EXPORT 开头。
                    ...
                    if collectExcelInfo.funsListtuple[i] in line_info_list:
                        ...
                        while "IN " in line_info or "OUT " in line_info or "INOUT " in line_info:
                            ...
                            line_info = file1.readline()
                            if not (line_info): break
                        dict_funs[collectExcelInfo.funsListtuple[i]] = temp_parameter_list  # 键值对
                    ...
                    while not (line_info2.startswith("SMEE_EXPORT")):  # 一直读到 SMEE_EXPORT 为止。
                        ...
                        # print("line 218,line_info = ",line_info)
                        if not (line_info): break
        elif CCNAME + '4T' == collectExcelInfo.funslist[i][0:4]:  # CC4T接口
            file2 = open((os.sep).join([filepath,"temp_HeaderFiles", "pre" + CCNAME + "4T_if.h"]), 'r')
            file2.seek(0)
