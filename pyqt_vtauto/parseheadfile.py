...
'''
# 判断temp_HeaderFiles 目录头文件收集是否齐全。
def check_headfile_all(filepath,headfile):
    headfile_path = (os.sep).join([filepath, headfile])
    if os.path.exists(headfile_path):
        if not headfile in headfile_list:  # 防止头文件重复读取。xx4a_if.h,xx4t.if.h可能都包含相同的头文件。
            ...
            file = open((os.sep).join([filepath, headfile]), 'r')
            file.seek(0)
...
def preprocess_header_files(CCNAME,filepath,existXX4A,existXX4T):
    ...
    filepath=filepath+"/temp_HeaderFiles"
    if existXX4A == True:
        file1 = open((os.sep).join([filepath, CCNAME + "4A_if.h"]), 'r')
        #os.chdir(r'XXVT')
        #print('os.chdir=',os.chdir)
        filePreHeader_4A = open((os.sep).join([filepath,"temp_pre" + CCNAME + "4A_if.h"]), 'w')
    ...
    if existXX4T == True:
        file2 = open((os.sep).join([filepath, CCNAME + "4T_if.h"]), 'r')

        filePreHeader_4T = open((os.sep).join([filepath,"temp_pre" + CCNAME + "4T_if.h"]), 'w')
...
# dict_funs，保存接口和参数信息。
def generate_requirefuns_tempfile(CCNAME, filepath,existXX4A,existXX4T,dict_pageToFuns,log):
    funslist = []
    ...
    file3 = open((os.sep).join([filepath,"temp_interfaces.txt"]), 'w')
    file2 = open((os.sep).join([filepath, "temp_HeaderFiles", "temp_pre" + CCNAME + "4T_if.h"]), 'r')
    file1 = open((os.sep).join([filepath, "temp_HeaderFiles", "temp_pre" + CCNAME + "4A_if.h"]), 'r')

    for i in range(len(funslist)):
        temp_parameter_list = []

        if CCNAME + '4A' == funslist[i][0:4]:  # CC4A接口
            ...
            while True:
                ...
                if line_info1.startswith('SMEE_EXPORT'):  # 找到以SMEE_EXPORT 开头。
                    ...
                    if tuple(funslist)[i] in line_info_list:
                        ...
                        while "IN " in line_info or "OUT " in line_info or "INOUT " in line_info:
                            ...
                            line_info = file1.readline()
                            if not (line_info): break
                        dict_funs[tuple(funslist)[i]] = temp_parameter_list  # 键值对
                    ...
                    while not (line_info2.startswith("SMEE_EXPORT")):  # 一直读到 SMEE_EXPORT 为止。
                        ...
                        # print("line 218,line_info = ",line_info)
                        if not (line_info): break
        elif CCNAME + '4T' == funslist[i][0:4]:  # CC4T接口
            file2.seek(0)
...
def collect_struct_enum(filepath, headfile_list,ExcelPath):
    file1 = open(ExcelPath+"/temp_StructEnumMacro.txt", 'w')
...
#--------------------------------------------------------------------
    #此段用于生成excel需求表格，暂且不动
    print("step .收集信息，利用dict_funs字典生成xml格式文件")
    genaral_xml(dict_struct, dict_enum,ExcelPath)
# 注意解析结构体和枚举,头文件统一在temp_HeaderFiles目录下。
# 生成 temp_funs.xml。
def genaral_xml(dict_struct, dict_enum,ExcelPath):
    curPath = ExcelPath
...
#temp_StructEnumMacro.txt为输入，
#生成temp_record_macro.txt，temp_dict_enum_struct.txt
def parse_temp_record_struct_enum(file1,ExcelPath):
    file1.close()#之前是写模式
    file1 = open(ExcelPath+"/temp_StructEnumMacro.txt", 'r')
    file2 = open(ExcelPath+"/temp_record_macro.txt", 'w')
    file3 = open(ExcelPath+"/temp_dict_enum_struct.txt", 'w')
    file1.seek(0)
    ...
    while temp_list_enum_struct.count('typedef enum') != 0:
        ...
        try:
            ...
            PyQt5.QtWidgets.QMessageBox.warning(None,"Illigal", "illigal enum:"+enum_MIN)
